      ...
      <main className="mx-auto w-full max-w-6xl px-4 pb-20 pt-10 sm:px-6 lg:px-8 lg:pt-16">
        <section className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
          <div>
            ...
            <p className="mt-4 text-base leading-7 text-slate-600 sm:text-lg">{t("public.demoPageSubtitle")}</p>

            <div className="mt-7 space-y-3">
              {features.map((feature) => (
                <p key={feature} className="flex items-start gap-3 text-sm text-slate-700">
                  <span className="mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-blue-100 text-xs font-semibold text-blue-700">✓</span>
                  <span>{feature}</span>
                </p>
              ))}
            </div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href="/explore"
                onClick={() => trackAnalyticsEvent("interactive_demo_opened", { source: "demo_page_cta" })}
              ...
              <Link
                href="/signup"
                onClick={() => trackAnalyticsEvent("start_trial_clicked", { source: "demo_page_cta" })}
              ...
              <Link
                href="/contact"
                onClick={() => trackAnalyticsEvent("book_demo_clicked", { source: "demo_page_cta" })}
          ...
          <DemoVideoEmbed
            ...
            className="lg:mt-2"
            onCompleted={() => trackAnalyticsEvent("demo_video_completed", { source: "demo_page" })}
          />
        </section>

        <section className="mt-16 rounded-3xl border border-slate-200 bg-white p-8">
          <p className="text-sm font-semibold uppercase tracking-[0.16em] text-blue-700">{t("public.demoOverview")}</p>
          <h2 className="mt-2 text-3xl font-semibold text-slate-950">{t("public.demoOverviewTitle")}</h2>
          <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-600 sm:text-base">{t("public.demoOverviewCopy")}</p>
        </section>
      </main>
