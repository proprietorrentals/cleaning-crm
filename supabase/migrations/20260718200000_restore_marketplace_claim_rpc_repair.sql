-- Additive repair: restore Lead Marketplace claim RPC expected by API.
-- API payload keys confirmed in src/app/api/super-admin/lead-marketplace/[leadId]/claim/route.ts:
--   target_lead_id, target_tenant_id, claiming_user_id, claiming_user_email
...
revoke all on function public.claim_marketplace_lead(text, uuid, uuid, uuid) from public;
revoke all on function public.claim_marketplace_lead(text, uuid, uuid, uuid) from anon;
grant execute on function public.claim_marketplace_lead(text, uuid, uuid, uuid) to authenticated;
...
--   and p.proname = 'claim_marketplace_lead';
